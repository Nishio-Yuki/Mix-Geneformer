...
class GeneformerPreCollator(SpecialTokensMixin):
    def __init__(self, *args, **kwargs) -> None:
        ...
        if pretext_task == "MLM" :
            ...
            self.model_input_names = ["input_ids", "labels", "token_type_ids"] #labels(next_sentelce_label), token_type_ids を追加 
        elif pretext_task == "BERT" :
            self.model_input_names = ["input_ids", "labels", "next_sentence_label", "token_type_ids"] #labels, next_sentence_label, token_type_ids を追加 
        elif pretext_task == "SimCSE" or pretext_task == "SimCSEpp" :
            self.model_input_names = ["input_ids"] 
        elif pretext_task == "MLM-SimCSE" or pretext_task == "MLM-SimCSEpp" :
            self.model_input_names = ["input_ids"]
        else :
            print("In GeneformerPreCollator. Do not specify Pre Text Task or must do debug to kwards!")
            sys.exit(1)
...
# BertPreTraining用のDataCollatorを追加（未完成：エラーが出た）
class DataCollatorForBertPreTraining(DataCollatorForLanguageModeling):
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        batch["next_sentence_label"] = torch.tensor([examples[i]["labels"] for i in range(len(examples))])
    ...
    def __init__(self, pretext_task="MLM", *args, **kwargs):
        ...
        if data_collator is None:
            ...
            if pretext_task == "MLM" :
                ...
                data_collator = DataCollatorForLanguageModeling( # Masked Language Modeling用のDataCollator
                    tokenizer=precollator, mlm=False, mlm_probability=0.0
                )
            elif pretext_task == "MLM-SimCSE" or pretext_task == "MLM-SimCSEpp" :
                data_collator = DataCollatorForLanguageModeling( # Masked Language Modeling用のDataCollator
                    tokenizer=precollator, mlm=True, mlm_probability=0.15
                ...
                print("In GeneformerPretrainer. Do not specify Pre Text Task or must do debug to kwards!")
                sys.exit(1)
