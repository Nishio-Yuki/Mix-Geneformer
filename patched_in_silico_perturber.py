...
def delete_indices(example):    
    indices = example["perturb_index"] # 削除するindex番号を格納
    if any(isinstance(el, list) for el in indices): # index番号がリストであることを確認
        indices = flatten_list(indices) # index番号のどれかがリストであるならば，1次元のリストにする
    for index in sorted(indices, reverse=True): # 1次元index番号リストの要素を大きい順に並び替え
        del example["input_ids"][index] # 大きいindex番号順に細胞文のindex番号を削除
    return example # 削除された細胞文を返す
# =====================================================================================

# =====================================================================================
# inhibit：遺伝子のランク値を下位に下げる(1-1/4)
...
def inhibit_indices(example):
    ...
    insert_num = (max_len - 1) + example["perturb_rank_direct_shift"] # example["perturb_rank_direct_shift"]には何番目にシフトするかの数値が格納
...
def inhibit_tokens(example):
    ...
    if example["perturb_index"] != [-100]: # 摂動するindex番号がある場合，細胞文を入力し，摂動するindex番号を削除した細胞文を格納
        example = delete_indices(example)
    max_len = len(example["input_ids"])
    insert_num = (max_len - 1) + example["shift_rank"] # example["shift_rank"]には何番目にシフトするかの数値が格納
    [example["input_ids"].insert(insert_num, token) for token in example["tokens_to_perturb"][::-1]] # 摂動するindex番号を削除した細胞文に摂動するトークンIDをIDが小さい順になるように追加
    return example # 摂動するトークンIDを追加した細胞文を返す
# =====================================================================================

# =====================================================================================
# overexpress：遺伝子のランク値を1にする
# for genes_to_perturb = "all" where only genes within cell are overexpressed
...
def overexpress_tokens(example):
    ...
    if example["perturb_index"] != [-100]: # 摂動するindex番号がある場合，細胞文を入力し，摂動するindex番号を削除した細胞文を格納
        example = delete_indices(example)
    [example["input_ids"].insert(0, token) for token in example["tokens_to_perturb"][::-1]] # 摂動するindex番号を削除した細胞文に摂動するトークンIDをIDが小さい順になるように追加
    return example # 摂動するトークンIDを先頭に追加した細胞文を返す
# =====================================================================================

# =====================================================================================
# active：遺伝子のランク値を上位に上げる(2048/4=512：0-511)
# for genes_to_perturb = "all" where only genes within cell are overexpressed
...
def active_tokens(example):
    ...
    if example["perturb_index"] != [-100]: # 摂動するindex番号がある場合，細胞文を入力し，摂動するindex番号を削除した細胞文を格納
        example = delete_indices(example)
    insert_num = example["shift_rank"] # example["shift_rank"]には何番目にシフトするかの数値が格納
    [example["input_ids"].insert(insert_num, token) for token in example["tokens_to_perturb"][::-1]] # 摂動するindex番号を削除した細胞文に摂動するトークンIDをIDが小さい順になるように追加
    return example # 摂動するトークンIDを先頭に追加した細胞文を返す
# =====================================================================================
...
def remove_indices_from_emb(emb, indices_to_remove, gene_dim):
    ...
    indices_to_keep = [i for i in range(emb.size()[gene_dim]) if i not in indices_to_remove] # 各細胞の摂動させないindex番号を格納
    num_dims = emb.dim() # 細胞文の次元数を格納（スカラー値）
    emb_slice = [slice(None) if dim != gene_dim else indices_to_keep for dim in range(num_dims)] # 0は細胞文，1がトークンだから，残すトークンだけ抽出するように細胞文をスライス
    sliced_emb = emb[emb_slice] # 残すトークンだけとなった細胞文の特徴量を作成
    #print("sliced_emb.shape: {}".format(sliced_emb.shape))
    return sliced_emb # 残すトークンだけとなった細胞文の特徴量を返す

def remove_indices_from_emb_batch(emb_batch, list_of_indices_to_remove, gene_dim):
    # 摂動前の各細胞文の特徴量と摂動前の各細胞文の摂動させるindex番号と次元数0を入力して，摂動前の摂動させるindex番号を削除した細胞文の特徴量を格納
    ...
    return output_batch # 摂動処理前の細胞文の特徴量から摂動させるindex番号を削除した細胞文の特徴量を返す
...
def make_perturbation_batch(example_cell, 
                            ...
                            combo_lvl,
                            num_proc):
    if tokens_to_perturb == "all": # 全ての遺伝子を1つずつ摂動する場合
        if perturb_type in ["overexpress"]:
            ...
            pass
        indices_to_perturb = [[i] for i in range(range_start, (example_cell["length"][0]-range_end))] # 全ての遺伝子 or 2番目の遺伝子から最後の遺伝子までのindex番号を格納
    elif combo_lvl>0 and (anchor_token is not None): 
        example_input_ids = example_cell["input_ids "][0] # 細胞文のトークン ID を全て取得  
        anchor_index = example_input_ids.index(anchor_token[0]) # アンカー遺伝子のトークン ID を取得
    ...
    if combo_lvl>0 and (anchor_token is None):
        if tokens_to_perturb != "all":
            ...
            all_indices = [index for index in all_indices if index not in indices_to_perturb] # all_indicesには多分何もはいてない→True
            indices_to_perturb = [[[j for i in indices_to_perturb for j in i], x] for x in all_indices] # 何もはいてない→True
    ...
    elif perturb_type in ("active","activate") :
...
def get_cell_state_avg_embs(model, # ここでstart stateとgoal stateの特徴量を獲得している
                            filtered_input_data,
    ...
    for possible_state in possible_states:
        ...
        avg_state_emb = torch.mean(avg_state_emb, dim=0, keepdim=True) # 各 state の特徴量の平均を計算し，これを各 state の特徴量としている
...
def quant_cos_sims(model, 
                   perturb_type,
                   perturbation_batch, # 摂動処理後の細胞文
                   perturb_rank_shift,
                   perturb_rank_direct_shift,
                   forward_batch_size, 
                   layer_to_quant, 
                   original_emb, # 摂動処理前の細胞文
                   tokens_to_perturb,
                   indices_to_perturb,
                   perturb_group,
                   cell_states_to_model,
                   ...
                   model_input_size,
                   nproc):
    cos = torch.nn.CosineSimilarity(dim=2) # コサイン類似度を定義
    total_batch_length = len(perturbation_batch) # 最大細胞数を格納

    if ((total_batch_length-1)/forward_batch_size).is_integer(): # うまく推論するための工夫（total_batch_length:4000, forward_batch_size:50時には関係ない）
        forward_batch_size = forward_batch_size-1
    
    if perturb_group == False: # 摂動する遺伝子を指定する場合，Trueとなる
        comparison_batch = make_comparison_batch(original_emb, indices_to_perturb, perturb_group)

    if cell_states_to_model is None: # ここは基本的に「not None」
        cos_sims = []
    else:
        possible_states = get_possible_states(cell_states_to_model)
    ...
    perturbation_batch = perturbation_batch.map( # 細胞文の長さを格納した細胞文データを格納
            measure_length, num_proc=nproc
        )

    def compute_batch_embeddings(minibatch, _max_len = None):
        minibatch_lengths = minibatch["length"] # 細胞文の長さを格納
        minibatch_length_set = set(minibatch_lengths) # 細胞文の長さをset化
        max_len = model_input_size # モデルの最大入力トークン数を格納

        if (len(minibatch_length_set) > 1) or (max(minibatch_length_set) > max_len): # paddingやtruncingが必要がどうかを判定
            needs_pad_or_trunc = True
        else:
            needs_pad_or_trunc = False
            max_len = max(minibatch_length_set) # ミニバッチ内で最大トークン数を格納
        if needs_pad_or_trunc == True: # ミニバッチ内でトークン数が揃っていない時にpadding などを実行
            if _max_len is None: # 入力次元数が2048となっているから，それを最大トークン数に変更する(perturbで行う)
                max_len = min(max(minibatch_length_set), max_len)
            else:
                if perturb_type == "overexpress" : # 入力次元数が2048となっているから，それを最大トークン数に変更する(originalで行う)
                    max_len = _max_len
        ...
        minibatch.set_format(type="torch") # ミニバッチのフォーマットをtorchにする
        
        input_data_minibatch = minibatch["input_ids"] # ミニバッチの細胞文を格納
        attention_mask = gen_attention_mask(minibatch, max_len) # ミニバッチ全ての細胞文と最大トークン数(同じサイズ)を入力し，attention maskを作成し格納
        
        # extract embeddings for perturbation minibatch
        with torch.no_grad(): # ミニバッチ全ての細胞文とattention maskを入力し，細胞文の埋め込み特徴量を格納
            outputs = model(
                # input_ids = input_data_minibatch.to("cuda"),
                input_ids = input_data_minibatch.to(ISP_device),
                attention_mask = attention_mask
            )
        return outputs, max_len # 細胞文の埋め込み特徴量と細胞文と最大トークン数を返す
    for i in range(0, total_batch_length, forward_batch_size): # 細胞文の推論開始
        max_range = min(i+forward_batch_size, total_batch_length) # 最大細胞数以上にならないように調整し，その値を格納
        perturbation_minibatch = perturbation_batch.select([i for i in range(i, max_range)]) # 摂動後の細胞文を選択しミニバッチとして格納
        outputs, mini_max_len = compute_batch_embeddings(perturbation_minibatch) # ミニバッチに分割した摂動処理後の細胞文を入力し，細胞文の埋め込み特徴量と最大トークン数を格納

        if len(indices_to_perturb)>1: # 摂動する遺伝子が格納されているindex番号が2つ以上存在（摂動処理前のindex番号）
            minibatch_emb = torch.squeeze(outputs.hidden_states[layer_to_quant])
        else:
            minibatch_emb = outputs.hidden_states[layer_to_quant]

        # overexpress は，摂動した遺伝子が残る．摂動前と後の遺伝子の cosime similarity を計算するときに残った遺伝子が邪魔となるため削除
        # delete は，摂動した遺伝子は残らない．inhibit，active は残るため，ここで遺伝子を削除
        if perturb_type == "overexpress":
            # remove overexpressed genes to quantify effect on remaining genes
            if perturb_group == False:
                overexpressed_to_remove = 1
            if perturb_group == True:
                # 正しい。細胞文中に存在しない遺伝子も先頭に挿入されているから。この場合、pad_or_truncate_encodingで2048に揃えられている。
                # 例：2個摂動で細胞文中に1個存在し，1個存在しない場合：2048→2047(1個削除)→2049(2個先頭に追加)→2048(pad_or_truncate_encodingで最後尾トークン削除(マスクで))
                overexpressed_to_remove = len(tokens_to_perturb) 
            minibatch_emb = minibatch_emb[:, overexpressed_to_remove: ,:]
        elif perturb_type in ("active","activate") :# 挿入したindex番号を消す
            if perturb_group == False :
                ...
                pass
        elif perturb_type == "inhibit" : # 挿入したindex番号を消す
            if perturb_group == False :
        ...
        if perturb_group == True:
            ...
            original_minibatch = original_emb.select([i for i in range(i, max_range)]) # 摂動処理前の細胞文からミニバッチ分の細胞文を抽出し格納
            original_outputs, orig_max_len = compute_batch_embeddings(original_minibatch, mini_max_len) # ミニバッチに分割した摂動処理前の細胞文を入力し，細胞文の埋め込み特徴量と最大トークン数を格納
            ...
            if len(tokens_to_perturb) == 1:
                ...
                for idx in indices_to_perturb[i:max_range]:
                    if idx == [-100]: # 摂動させるindex番号がないoverexpressの場合
                        indices_to_perturb_minibatch.append(end_range)
                    else: # 摂動させるindex番号がある場合
                        if len(idx) == num_perturbed : # 摂動前の摂動させるindex番号の総数が摂動させるトークンID数と同じ場合
                            indices_to_perturb_minibatch.append(idx) # 摂動前のindex番号を格納
                        elif len(idx) < num_perturbed : # 摂動前の摂動させるindex番号の総数が摂動させるトークンID数未満の場合
                            indices_to_perturb_minibatch.append(idx + end_range[-(num_perturbed-len(idx)):]) # 摂動前のindex番号と細胞文の最後のindex番号を格納．inhibit も active もこの処理でOK．pad_or_truncate_encodingで最後尾をマスクしているから．
                        else : # それ以外の場合，error
                            if len(idx) == 0 :
            ...
            # 摂動前の細胞文の特徴量と摂動前の摂動させるindex番号と次元数1を入力し，摂動前の細胞文の特徴量から摂動させるindex番号を削除した細胞文の特徴量を格納
            # （摂動前の細胞文と摂動後の細胞文の cosine similarity を取るために，次元数を揃える必要がある．
            #   そのため，摂動前の細胞文の特徴量である original_emb は摂動した分消去する必要がある．そこで，摂動した遺伝子を削除することで帳尻を合わせている）
        ...
        if cell_states_to_model is None: # cell_states_to_modelはstart_stateとかgoal_stateが格納されたdictのこと．絶対ある
            if perturb_group == False:
            ...
            for state in possible_states:
                if perturb_group == False: # ここでstart stateとgoal stateの特徴量と摂動した細胞文の特徴量とのコサイン類似度を計算（順番に）
...
class InSilicoPerturber:
    valid_option_dict = {
        ...
        "perturb_rank_direct_shift": {None, int},
    ...
    def validate_options(self):
        ...
        if self.perturb_type in ["inhibit", "activate"]:
            if (self.perturb_rank_shift is None) and (self.perturb_rank_direct_shift is None) :
                ...
                elif self.perturb_type in ("active","activate"):
                    logger.warning(
    ...
    def perturb_data(self, 
        ...
        filtered_input_data = load_and_filter(self.filter_data, self.nproc, input_data_file) # データをロードしてフィルタを適用
        model = load_model(self.model_type, self.num_classes, model_directory) # モデルをロード
        layer_to_quant = quant_layers(model)+self.emb_layer # ??
        
        if self.cell_states_to_model is None: # これはNoneになることは殆ど無い
            state_embs_dict = None
        else:
            # confirm that all states are valid to prevent futile filtering
            # start stateの状態を取得し，入力するデータの抽出
            ...
            # 抽出したデータにstart stateが存在するかをフィルタリングする
    ...
    def in_silico_perturb(self,
        ...
        output_path_prefix = f"{output_directory}in_silico_{self.perturb_type}_{output_prefix}_dict_1Kbatch" # 出力するファイル名を決定
        model_input_size = get_model_input_size(model) # モデルサイズを格納
        
        # filter dataset for cells that have tokens to be perturbed
        if self.anchor_token is not None: # anchor_tokenが存在する場合実行（5/9時点ではNone）
            def if_has_tokens_to_perturb(example): # anchor_tokenが存在する細胞のみを抽出
            ...
            if len(filtered_input_data) == 0: # 細胞が存在しない場合
                logger.error(
                        "No cells in dataset contain anchor gene.")
                raise
            else:
                logger.info(f"# cells with anchor gene: {len(filtered_input_data)}") # 細胞が存在する場合，細胞数を表示
        ...
        if (self.tokens_to_perturb != "all") and (self.perturb_type != "overexpress") and (self.perturb_type != "active") and (self.perturb_type != "inhibit"): # delete の時に実行
            ...
            min_genes = len(self.tokens_to_perturb) # 摂動するトークン ID 数を格納
            
            def if_has_tokens_to_perturb(example): # 摂動するトークン ID が全てある細胞を抽出
                return (len(set(example["input_ids"]).intersection(self.tokens_to_perturb))>=min_genes)
            filtered_input_data = filtered_input_data.filter(if_has_tokens_to_perturb, num_proc=self.nproc)
            if len(filtered_input_data) == 0: # 細胞がなかったら実行
                logger.error(
                        "No cells in dataset contain all genes to perturb as a group.")
                raise
            
        cos_sims_dict = defaultdict(list) # 辞書型の初期化
        pickle_batch = -1
        filtered_input_data = downsample_and_sort(filtered_input_data, self.max_ncells) # 総細胞数から摂動に使用する細胞数にダウンサンプリングする
        if self.cell_inds_to_perturb != "all": # 複数のGPUに分ける場合（5/9時点では分けない）
        ...
        if self.perturb_group == True: # 摂動する遺伝子を指定している場合
                
            def make_group_perturbation_batch(example): # 細胞文の摂動を実行
                example_input_ids = example["input_ids"] # 摂動させる細胞文を格納
                example["tokens_to_perturb"] = self.tokens_to_perturb # 摂動させる遺伝子のトークンIDを"token_to_perturb"に格納（全細胞共通）
                # 摂動させるトークンIDが格納されているindex番号を格納．なければNoneを格納
                ...
                indices_to_perturb = [item for item in indices_to_perturb if item is not None] # indices_to_perturbからNoneを削除し，index番号のみにする
                if len(indices_to_perturb) > 0: # indices_to_perturb に index 番号がある場合
                    example["perturb_index"] = indices_to_perturb # 摂動させる遺伝子が存在するから，そのindex番号を"perturb_index"に格納
                else: # indices_to_perturb に index 番号がない場合
                    # -100 indicates tokens to overexpress are not present in rank value encoding
                    # 摂動が overexpress と active で，摂動する遺伝子が細胞文中に存在しない場合，"perturb_index"に-100を格納
                    example["perturb_index"] = [-100]
                if self.perturb_type in ("active","activate") : # inhibit か active 時に，perturb_rank_direct_shift を各細胞の情報に追加
                    if (self.perturb_rank_direct_shift is not None) and (self.perturb_rank_shift is None) :
                        ...
                        example["shift_rank"] = self.perturb_rank_direct_shift # ここには index 番号（shift にあった）を入れる
                    else :
                        print("Error")
                        print("You must choice one perturb shift type, direct(perturb_rank_direct_shift) or indirect(perturb_rank_shift). Not both.")
                        sys.exit(1)
                    ...
                    pass
                if self.perturb_type == "delete": # delete摂動を行う場合，細胞文を入力し，摂動をするindex番号を削除した細胞文を格納
                    example = delete_indices(example)
                elif self.perturb_type == "overexpress": # overexpress摂動を行う場合，細胞文を入力し，摂動するトークンIDを先頭に追加した細胞文を格納
                    ...
                    pass
                return example 
            perturbation_batch = filtered_input_data.map(make_group_perturbation_batch, num_proc=self.nproc) # 摂動処理後の細胞文を格納
            indices_to_perturb = perturbation_batch["perturb_index"] # 摂動する遺伝子が格納されているindex番号を格納（摂動処理前のindex番号）
